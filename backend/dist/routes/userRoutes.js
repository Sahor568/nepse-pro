...
router.put('/profile', async (req, res) => {
    try {
        ...
        const db = await getDb();
        await db.run('UPDATE users SET name = ?, email = ? WHERE id = ?', [name, email, req.userId]);
...
router.post('/portfolio', async (req, res) => {
    try {
        ...
        const db = await getDb();
        const result = await db.run('INSERT INTO portfolio (user_id, symbol, quantity, buy_price, buy_date, reference) VALUES (?, ?, ?, ?, ?, ?)', [req.userId, symbol, quantity, buy_price, buy_date, reference || '']);
...
router.put('/portfolio/:id', async (req, res) => {
    try {
        ...
        const db = await getDb();
        await db.run('UPDATE portfolio SET quantity = ?, buy_price = ?, buy_date = ?, reference = ? WHERE id = ? AND user_id = ?', [quantity, buy_price, buy_date, reference || '', id, req.userId]);
